...
def number_squared(num: int) -> int:
    ...
    Args:
        num (int): the number to number_squared
    
    Returns:
        int: the number to squared
    """
    return num ** 2 
...
def string_repeater(str: str, num:int):
    """takes in a string and returns a longer string witht hte original string repeated multiple times
...
def jacket_checker(temp:int, raining:bool) -> str:
    """takes in a temperature and whether it is raining and returns whether or not to wear a jacket

    Args:
        temp (int): temperature outside
        raining (bool): whether or not it is raining
...
def round_down(num: float) -> int:
    """takes in a float and returns a numbers rounded down to the nearest whole number
