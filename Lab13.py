...
class Customer:
    ...
    def add_to_cart(self, product: Product) -> None:
        ...
        Args:
            product (Product): Products to add
        """
    ...
    def checkout(self) -> None:
        """ Calculate the total price of all the products in the cart and print out the total. Empty the cart.
        """
        total = 0
    ...
    def display_products(self) -> None:
        """Print all the products in the cart list. """
        print(f"{self.name}'s cart:")
        for product in self.cart:
            print(product)

    def display_products_pretty(self) -> None:
        """Print all the products in the cart list in a pretty table. """
        from tabulate import tabulate
       
        print(f"{self.name}'s cart:")
        print(tabulate([{"Name": product.name, "Price": product.price, "ID": product.product_id} for product in self.cart], headers="keys", tablefmt="fancy_grid"))


class Store:
    """Store class that has a list of products and customers."""
    ...
    def add_product(self, product: Product) -> None:
        ...
        print(f"{product} was addded to the store.")
    ...
    def display_products(self) -> None:
        """Print all the products in the products list. """
        for product in self.products:
            print(product)

    def display_customers(self) -> None:
        """Print all the customers in the customers list. """
...
# 4. Create a main function that will call the menu function and then call the appropriate methods based on the user's choice. The main function should be in a while loop that will continue to call the menu function until the user enters 9 to exit the program.
...
def main():
    ...
    while True:
        ...
        if choice ==1:
            ...
            for customer in store.customers:
                if customer.customer_id == customer_id:
                    customer.display_product_pretty()
        elif choice == 8:
            customer_id = int(input("Enter customer ID: "))
            for customer in store.customers:
                if customer.customer_id == customer_id:
